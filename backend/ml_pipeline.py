                ...
                if current_video_time - vdata['first_seen'] >= PARKING_TIME_THRESHOLD:
                    ...
                    if not is_duplicate:
                        ...
                        new_record = models.ViolationRecord(
                            timestamp=timestamp_dt, violation_type="Illegal Parking",
                        ...
                        await websocket_manager.broadcast({
                            "type": "violation", "violation_type": "Illegal Parking", "severity": "MINOR",
            ...
            for vid, vdata in active_vehicles.items():
                ...
                if not check_seatbelt(frame, vdata['box']):
                    ...
                    if not is_dup:
                        ...
                        new_record = models.ViolationRecord(
                            timestamp=timestamp_dt, violation_type="Seatbelt Non-compliance", severity=severity,
                ...
                if is_wrong_side:
                    ...
                    if not is_dup:
                        ...
                        new_record = models.ViolationRecord(
                            timestamp=timestamp_dt, violation_type="Wrong-side Driving", severity=severity,
                            rider_count=0, plate_number=plate_number, confidence=v_conf, image_url=f"/api/images/{filename}"
                        )
                        db.add(new_record)
                        total_violations += 1
                        ...
                        await websocket_manager.broadcast({
                            "type": "violation", "violation_type": "Wrong-side Driving", "severity": severity,
                            "plate_number": plate_number, "confidence": float(v_conf), "rider_count": 0,
                            "image_url": f"/api/images/{filename}", "timestamp": timestamp_dt.isoformat()
                        })
                ...
                if TRAFFIC_LIGHT_STATE == "RED":
                    if vy2 > STOP_LINE_Y:
                        ...
                        if not is_dup:
                            ...
                            new_record = models.ViolationRecord(
                                timestamp=timestamp_dt, violation_type=v_type, severity=severity,
                                rider_count=0, plate_number=plate_number, confidence=v_conf, image_url=f"/api/images/{filename}"
                            )
                            db.add(new_record)
                            total_violations += 1
                            ...
                            await websocket_manager.broadcast({
                                "type": "violation", "violation_type": v_type, "severity": severity,
                                "plate_number": plate_number, "confidence": float(v_conf), "rider_count": 0,
                                "image_url": f"/api/images/{filename}", "timestamp": timestamp_dt.isoformat()
                            })
            ...
            for moto_box, moto_conf, moto_id in tracked_motorcycles:
                ...
                if rider_count >= TRIPLE_RIDING_THRESHOLD:
                    ...
                    if not is_duplicate:
                        ...
                        new_record = models.ViolationRecord(
                            timestamp=timestamp_dt, violation_type="Triple Riding", severity=severity,
                        ...
                        await websocket_manager.broadcast({
                            "type": "violation", "violation_type": "Triple Riding", "severity": severity,
                ...
                if no_helmet_riders:
                    ...
                    if not is_duplicate:
                        ...
                        new_record = models.ViolationRecord(
                            timestamp=timestamp_dt, violation_type="Helmet Non-compliance", severity=severity,
